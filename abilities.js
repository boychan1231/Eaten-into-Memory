...
function activateSeaPreRoundAbility(gameState) {
    ...
    if (seaPlayer && gameState.roundMarker === 1) {
        if (seaPlayer.mana >= 2 && seaPlayer.hand.length >= 1) {
            if (Math.random() < 0.5) { 
                seaPlayer.mana -= 2;
                
                const cardToDiscardIndex = seaPlayer.hand.findIndex(c => c.value === Math.min(...seaPlayer.hand.map(c => c.value)));
                const discardedCard = seaPlayer.hand.splice(cardToDiscardIndex, 1)[0];
                gameState.minuteDiscard.push(discardedCard);
                
                console.log(`【時之惡】耗用 2 Mana 並捨棄 ${discardedCard.value} 號卡，發動回合開始前能力。`);

				/*【暫移除】 時之惡捨棄受詛者贈卡
                gameState.players.filter(p => p.type !== '受詛者').forEach(player => {
                    const giftCardIndex = player.hand.findIndex(c => c.isGift);
                    if (giftCardIndex !== -1) {
                        const giftCard = player.hand.splice(giftCardIndex, 1)[0];
                        gameState.minuteDiscard.push(giftCard);
                        console.log(`【時之惡能力】${player.name} 捨棄了 受詛者贈卡 (${giftCard.value})。`);
                    }
                });
				*/
            }
